...
LayerLys::LayerLys() : lys::Layer("Lys"), m_Next_State(-1), m_Condition_Change_State(false), m_Remaining_Time(2.0f) {}
...
void LayerLys::mt_On_Render(void)
{
    ...
    Renderer::smt_Get().mt_Draw_Text("Lys", Vector2f(Window::smt_Get().mt_Get_Size().x * 0.5f, Window::smt_Get().mt_Get_Size().y * 0.66f),
                                     l_Text_Settings);
}

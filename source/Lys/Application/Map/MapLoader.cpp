...
bool MapLoader_Tiled_1_9::mt_Load(const File& file_path, MapData& map_data, std::unique_ptr<MapLayout>& map_layout)
{
    ...
    l_Loader.mt_Add_On_Entry_Callback("/map/tileset", [&](const XML_Element& tileset)
    {
        ...
        if (tileset.mt_Get_Attribute("firstgid", l_Tileset_Data.m_First_Gid) == false)
        {
            return false;
        }
        ...
        if (mt_Load_Tileset(file_path.mt_Get_Path() + l_Source_File, l_Tileset_Data) == false)
        {
            return false;
        }
    ...
    l_Loader.mt_Add_On_Entry_Callback("/map/layer", [&](const XML_Element& layer) { return mt_Load_Layer(layer); });
    l_Loader.mt_Add_On_Entry_Callback("/map/layer/data", [&](const XML_Element& layer) { return mt_Load_Layer_Data(layer); });
    ...
    l_Loader.mt_Add_On_Entry_Callback("/map/objectgroup/object", [&](const XML_Element& object) { return mt_Load_Object(object); });
    l_Loader.mt_Add_On_Entry_Callback("/map/objectgroup/object/polygon",
                                      [&](const XML_Element& polygon) { return mt_Load_Object_Polygon(polygon); });
    l_Loader.mt_Add_On_Entry_Callback("/map/objectgroup/object/properties/property",
                                      [&](const XML_Element& object) { return mt_Load_Object_Property(object); });
